...
$gametypes = GetGameTypesList($gametype, 1, "SELECT game_type FROM $stats_m_table GROUP BY game_type ORDER BY game_type");
$servers   = GetServersList($server, 1, "SELECT $servers_table.id, $servers_table.name FROM $servers_table, $stats_m_table WHERE $servers_table.id=$stats_m_table.server GROUP BY $servers_table.id ORDER BY $servers_table.name");

$conditions = "WHERE $players_table.id=$stats_m_table.player AND time>$averagelimit ";
...
$query = DBQuery("SELECT 
                    ...
                    SUM($stats_m_table.kills)*$timefactor/time         AS avg_kills       ,
                    SUM($stats_m_table.deaths)*$timefactor/time        AS avg_deaths      ,
                    SUM($stats_m_table.murders)*$timefactor/time       AS avg_murders     ,
                    SUM($stats_m_table.suicides)*$timefactor/time      AS avg_suicides    ,
                    SUM($stats_m_table.headshots)*$timefactor/time     AS avg_headshots   ,
                    SUM($stats_m_table.knifings)*$timefactor/time      AS avg_knifings    ,
					SUM($stats_m_table.sniper_kills)*$timefactor/time  AS avg_sniper_kills,
                    SUM($stats_m_table.medic_saves)*$timefactor/time   AS avg_medic_saves ,
                    SUM($stats_m_table.revives)*$timefactor/time       AS avg_revives     ,
                    SUM($stats_m_table.doublekills)*$timefactor/time   AS avg_doublekills ,
...
<div style="visibility:hidden; font-size:xx-small;">monthlystats/modules/kill_stats.php 1.0.0</div>