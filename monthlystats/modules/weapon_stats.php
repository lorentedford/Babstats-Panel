...
$gametypes = GetGameTypesList($gametype, 1, "SELECT $stats_m_table.game_type FROM $stats_m_table, $weaponstats_m_table WHERE $stats_m_table.id=$weaponstats_m_table.record GROUP BY $stats_m_table.game_type ORDER BY $stats_m_table.game_type");
$servers   = GetServersList($server, 1, "SELECT $servers_table.id, $servers_table.name FROM $servers_table, $stats_m_table, $weaponstats_m_table WHERE $servers_table.id=$stats_m_table.server AND $weaponstats_m_table.record=$stats_m_table.id GROUP BY $servers_table.id ORDER BY $servers_table.name");

$conditions = "WHERE ";

$conditions .= "$stats_m_table.id=$weaponstats_m_table.record ";
...
$query = DBQuery("SELECT 
					...
					$weaponstats_m_table.weapon                                              AS weapon      ,
...
<div style="visibility:hidden; font-size:xx-small;">monthlystats/modules/weapon_stats.php 1.0.0</div>