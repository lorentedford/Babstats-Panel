...
$gametypes = GetGameTypesList($gametype, 1, "SELECT $stats_m_table.game_type FROM $stats_m_table, $players_table WHERE $stats_m_table.player=$players_table.id AND $players_table.squad='$id' GROUP BY $stats_m_table.game_type ORDER BY $stats_m_table.game_type");
$servers   = GetServersList($server, 1, "SELECT $servers_table.id, $servers_table.name FROM $servers_table, $stats_m_table, $players_table WHERE $servers_table.id=$stats_m_table.server AND $players_table.id=$stats_m_table.player AND $players_table.squad='$id' GROUP BY $servers_table.id ORDER BY $servers_table.name");

$conditions  = "WHERE $players_table.squad='$id'"; 
$conditions .= " AND $stats_m_table.player=$players_table.id ";
...
$query = DBQuery("SELECT 
                  ...
                  FROM $players_table, $stats_m_table, $squads_table
                  $conditions
...
if($server   != -1)    $conditions3 .= "AND $stats_m_table.server='$server' ";
if($gametype != "All") $conditions3 .= "AND $stats_m_table.game_type='$gametype' ";
...
$sniper_sql = DBQuery("SELECT $players_table.id, $players_table.squad, $stats_m_table.player, $stats_m_table.id, 
                         $weaponstats_m_table.id, SUM($weaponstats_m_table.kills) AS sniper_kills, 
					     SUM($weaponstats_m_table.kills)*$timefactor/SUM($weaponstats_m_table.time) AS avg_sniper_kills 
                       FROM $players_table, $stats_m_table, $weaponstats_m_table, $weapons_table
                       WHERE $weaponstats_m_table.record = $stats_m_table.id 
					   AND $stats_m_table.player = $players_table.id AND $players_table.squad = '$id'  
...
if(file_exists($base_folder."templates/squad_stats.htm")) {
  ...
  foreach($types as $value) {
      $row["$value"] = GetGameLevels($id, $value, "squad", "month");
  }
  ...
  while($player = DBFetchArray($player_query)) {
	...
	foreach($plyr as $pvalue) {
	  $plyrs_m_query = DBQuery("SELECT 
                          ...
                          FROM $stats_table 
                          WHERE $stats_table.player = '$pvalue'
						  $conditions2
...
<div style="visibility:hidden; font-size:xx-small;">monthlystats/modules/squad_details.php 1.0.0</div>