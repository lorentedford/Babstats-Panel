...
$servers = GetServersList($server, 1, "SELECT $servers_table.id, $servers_table.name FROM $servers_table, $maps_table, $stats_table, $mapstats_table WHERE $maps_table.id='$id' AND $servers_table.id=$stats_table.server AND $maps_table.id=$mapstats_table.map AND $stats_table.id=$mapstats_table.record GROUP BY $stats_table.server ORDER BY $servers_table.name");
...
$query = DBQuery("SELECT 
                    ...
                    SUM($mapstats_table.score)                                AS score         ,
                    SUM($mapstats_table.kills)*$timefactor/$maps_table.time   AS avg_kills     ,
                    SUM($mapstats_table.score)*$timefactor/$maps_table.time   AS avg_score     ,
...
$conditions  = "WHERE $mapstats_table.map='$id' "; 
...
$player_query = DBQuery("SELECT
                           ...
                           SUM($mapstats_table.score)*$timefactor/SUM($mapstats_table.time)  AS avg_score   
                         FROM $maps_table, $mapstats_table, $players_table, $stats_table
                         $conditions
                         GROUP BY $mapstats_table.record
                         ORDER BY time DESC
                         LIMIT 25");
...
<div style="visibility:hidden; font-size:xx-small;">modules/map_details.php 1.0.0</div>