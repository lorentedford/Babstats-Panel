...
$gametypes = GetGameTypesList($gametype, 1, "SELECT $stats_table.game_type FROM $stats_table, $weaponstats_table WHERE $stats_table.id=$weaponstats_table.record AND $weaponstats_table.weapon='$id' GROUP BY $stats_table.game_type ORDER BY $stats_table.game_type");
$servers   = GetServersList($server, 1, "SELECT $servers_table.id, $servers_table.name FROM $servers_table, $stats_table, $weaponstats_table WHERE $servers_table.id=$stats_table.server AND $weaponstats_table.record=$stats_table.id AND $weaponstats_table.weapon='$id' GROUP BY $servers_table.id ORDER BY $servers_table.name");
...
$conditions  = "WHERE $weaponstats_table.weapon='$id' "; 
...
$player_query = DBQuery("SELECT
                           ...
                           SUM($weaponstats_table.kills)*$timefactor/SUM($weaponstats_table.time) AS avg_kills  ,
                           SUM($weaponstats_table.kills)*100/if(SUM($weaponstats_table.shots)=0, 1, SUM($weaponstats_table.shots)) AS accuracy 
                         FROM $weapons_table, $weaponstats_table, $players_table, $stats_table
                         $conditions
...
<div style="visibility:hidden; font-size:xx-small;">modules/weapon_details.php 1.0.0</div>